...
# ------------------------------------- Functions ---------------------------------------
# def create_SECT_list(data_path):

#     data_list = os.path.join(os.getcwd(),data_path + 'CT/')
#     data_list = os.listdir(data_list)

#     list_all = [{'data': os.path.join(data_path + 'baseline-CT/', path), 'label': os.path.join(data_path + 'follow-up-CT/', path)} for path in data_list]
...
class NifitDataSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.transforms:  # apply the transforms to image and label (normalizSCANation, resampling, patches)
        ...
        return torch.from_numpy(image_np), torch.from_numpy(label_np), tempfilename  # this is the final output to feed the network

    def __len__(self):
        return len(self.data_list)

class SECT_NifitDataSet(torch.utils.data.Dataset):
